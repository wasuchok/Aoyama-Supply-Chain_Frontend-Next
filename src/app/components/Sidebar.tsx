...
const Sidebar = ({ isOpen, onToggle, activeMenu, onMenuClick }: any) => {
    return (
        <div
            className={`fixed inset-y-0 left-0 z-50 w-64 bg-white border lg:rounded-xl border-gray-100 transform ${isOpen ? 'translate-x-0' : '-translate-x-full'
        ...
        >
            ...
            <nav className="mt-8 px-3 space-y-3">
                ...
                <a
                    ...
                    className={`flex items-center px-3 py-3 text-base rounded-md hover:bg-primary-50 hover:text-primary-600 transition-all duration-200 group ${activeMenu === 'survey'
                            ? 'bg-primary-50 text-primary-600 border-r-2 border-primary-600 shadow-sm'
                            : 'text-gray-600 hover:shadow-md'
                        }`}
                >
                    <FaClipboardList className={`w-5 h-5 mr-3 flex-shrink-0 transition-colors ${activeMenu === 'survey' ? 'text-primary-500' : 'text-gray-400 group-hover:text-primary-500'
                        }`} />
                    Survey
                </a>
                <a
